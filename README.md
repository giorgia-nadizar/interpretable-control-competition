...
For the sake of completeness and to enable you to inspect the PDDL files directly, this .sh script also automatically generates the PDDL files associated with the problem (the domain file and the instance file). These .pddl files won't be read by the simulation script, since it will only ready the JSON file and internally encode the instance in PDDL without saving additional files on the system. These PDDL files are saved only to enable you to directly inspect them for gathering further insights into this formalism within our specific domain. These PDDL files are locally generated by using `json2PDDL.py` within this .sh script.
...
- `State` represents the current observation of the environment. It is composed of two attributes: atoms and fluents. The atoms are the predicates, while the fluents are the functions or actions that can be performed to transform this state into another one. In particular, the atoms attribute is organized as a collection (tuple) of several elements, where each element is a collection (tuple) of predicates, where each predicate is a tuple of IDs where the first ID refers to the predicate name while the other IDs refer to the predicate arguments. If the predicate is listed, it means that it holds true. The attribute fluents is organized as a collection (tuple) of elements, where each element is a function, that is, a pair (tuple), where the fist value is the function ID and the second value is a tuple of IDs indicating the arguments of the function. To avoid redundancy, in practice, this fluents attribute is usually empty. To get the actions that are available for the given state, please use the `get_available_actions` method.
- `Action` represents the action chosen by the controller. It is composed of two attributes: action_id and args. The action_id is the ID of the action, while the args are the IDs of the arguments.
- Available actions for a given state can be obtained via `get_available_actions`, which outputs a list of actions.

Both State and Action expose an attribute domain. IDs are integers.
