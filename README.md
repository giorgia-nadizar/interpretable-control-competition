...
- Currently, the folder includes an example file (`example.json`), which has been generated with:

```bash
./generate_json_problem_instance.sh 2026 50.0 0 150 example.json
```

This example problem instance contains 150 flights, and it is extremely challenging.
We provide in the problems folder additional problem instances with complexity from small to large. These instances have been generated by executing the script `generate_small_medium_large_instances.sh`.

During the evaluation phase, to assess generalization, we will **not** use these instances to test your policy. However, we will use other, hidden instances that are similar to the instances provided by `generate_small_medium_large_instances.sh`.
...
In the simulation script, JSON problem instances will be used to encode the problem by using the classical PDDL (Planning Domain Description Language) formalism (version of PDDL is at least 2.1).
